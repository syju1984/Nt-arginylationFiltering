...
##241015 MS2 transfer learning for EDC Sidechain E coli
from peptdeep.settings import *
# add_new_modifications(
    # new_mods= [
        # ("EDC_Tyr@Y", "C(8)H(17)N(3)"),
        # ("SideArgEthanolamine@D", "C(8)H(17)N(5)O(1)"),
        # ("SideArgEthanolamine@E", "C(8)H(5)N(5)O(1)"),
        # ("Ethanolamine@Any C-term", "C(2)H(5)N(1)"),
    # ]
# )
# update_modifications()

import pandas as pd
precursor_df = pd.read_table('./arg_inrich_tandem/input/precursor_df_311547_wo_arg.tsv')
fragment_intensity_df = pd.read_table('./arg_inrich_tandem/input/precursor_df_311547_wo_arg_fragdf.tsv')
precursor_df = precursor_df.fillna("") ### this code is important!!!!!!!
...
model_mgr.epoch_to_train_ms2 = 100
model_mgr.train_ms2_model(psm_df=precursor_df,matched_intensity_df=fragment_intensity_df)

validation_df = pd.read_table('./arg_inrich_tandem/input/validation.tsv')
validation_fragment_intensity_df = pd.read_table('./arg_inrich_tandem/input/validation_frag_df.tsv')
validation_df = validation_df.fillna("") ### this code is important!!!!!!!

validation_predict_inten_df=model_mgr.predict_ms2(validation_df)
metrics_describ = calc_ms2_similarity(
    validation_df, 
    predict_intensity_df=validation_predict_inten_df, 
    fragment_intensity_df=validation_fragment_intensity_df,
    spc_top_k=0
)
metrics_describ[0].to_csv("./arg_inrich_tandem/output/ms2_prediction_output_finetuned_model_trypsin_prediction_result.tsv",sep="\t")
metrics_describ[1].to_csv("./arg_inrich_tandem/output/ms2_prediction_summary_finetuned_model_trypsin_prediction_result.tsv",sep="\t")
predict_inten_df.to_csv("./arg_inrich_tandem/output/ms2_prediction_predictintendf_finetuned_model_trypsin_prediction_result.tsv",sep="\t")
